...
public interface ServisRepository  extends JpaRepository<Servis, Long> {
	...
	@Query("select s from Servis s where s.vozilo.vin=:vin and YEAR(s.datum)=:year and MONTH(s.datum)=:month order by s.datum DESC")
	List<Servis> getCustomServis(@Param("vin") String vin, @Param("year") int year, @Param("month") int month);
